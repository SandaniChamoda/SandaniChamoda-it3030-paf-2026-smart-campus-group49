...
function ResourceList() {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [search, filterType, capacityFilter]);
  ...
  const resolveImageGroup = (type) => {
    const normalizedType = String(type || "").toUpperCase();

    if (normalizedType.includes("LAB")) {
      return themedImageCatalog.LAB;
    }

    if (normalizedType.includes("ROOM")) {
      return themedImageCatalog.ROOM;
    }
  ...
  const getCardImage = (resource) => {
    const imageGroup = resolveImageGroup(resource.type);
  ...
  const filteredResources = useMemo(() => {
    return resources.filter((r) => {
      ...
      return (
        ...
        capacityMatches(Number(r.capacity) || 0)
      );
    });
  }, [resources, search, filterType, capacityFilter]);
          ...
          return (
            <article className="resource-card" key={r.id}>
              ...
              <div className="resource-card-body">
                <p className="resource-type">{toTitleCase(r.type)}</p>
                <h3>{r.name}</h3>
